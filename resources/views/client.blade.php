            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100 flex justify-between items-center">
                    ...
                    <button type="button" onclick="openModal()"
                        class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full">Add
                        Client</button>
                </div>
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <table id="{{ $dataTable->getTableAttribute('id') }}"
    ...
    @push('scripts')
        ...
        <script type="module">
                ...
                if ($id) {
                    axios.get('/clients/' + $id)
                        .then(response => {
                            ...
                            modalInputPhone.val(response.data.phone);
            ...
            window.submitForm = function($id = null) {
                var form = $('#userForm');
                if ($id) {
                    axios.put('/clients/' + $id, $(form).serialize())
                        .then(response => {
                            if (response.data.status) {
                                staticModal.hide();

                                let relode = $('#clients-table').DataTable();
                                relode.ajax.reload();
